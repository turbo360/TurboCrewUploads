...
import { formatFileSize, formatDuration, formatTimeOfDay } from '../utils/format';
  ...
  return (
    <div className="bg-gray-900/70 backdrop-blur-sm rounded-lg border border-gray-700/50 overflow-hidden">
      ...
      >
        ...
        <span className="text-xs text-gray-400">{formatTimeOfDay(batch.completedAt)}</span>

        <div className="flex-1" />

        <span className="text-xs text-gray-400">{batch.fileCount} files</span>
        <span className="text-xs text-gray-400">{formatFileSize(batch.totalBytes)}</span>
      ...
      {expanded && (
        <div className="px-4 pb-3 border-t border-gray-700/50 pt-3">
          <div className="flex items-center gap-4 text-xs text-gray-400">
            ...
            <span>{formatFileSize(batch.totalBytes)} transferred</span>
          </div>
        </div>
      )}
    </div>
